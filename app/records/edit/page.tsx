...
export default function Page() {
  ...
  const [sortKey, setSortKey] = useState<keyof Record>("time_ms")
            ...
            {sortedRecords.map((record) => (
              <tr key={record.id}>
                <td className="px-2 py-1">
                  <input
                    className="w-28 px-1"
                    value={record.timeStr}
                    onChange={e => {
                      const newVal = e.target.value
                      setRecords(prev => prev.map(r => r.id === record.id ? { ...r, timeStr: newVal } : r))
                    }}
                  />
                </td>
                <td className="px-2 py-1">
                  <input
                    ...
                    value={record.race_name}
                    onChange={e => {
                      const newVal = e.target.value
                      setRecords(prev => prev.map(r => r.id === record.id ? { ...r, race_name: newVal } : r))
                    }}
                  />
                </td>
                <td className="px-2 py-1">
                  <input
                    ...
                    onChange={e => {
                      const newVal = e.target.value
                      setRecords(prev => prev.map(r => r.id === record.id ? { ...r, distance: Number(newVal) } : r))
                    }}
                  />
                </td>
                <td className="px-2 py-1">
                  <select
                    value={record.race_type}
                    onChange={e => {
                      const newVal = e.target.value
                      setRecords(prev => prev.map(r => r.id === record.id ? { ...r, race_type: newVal } : r))
                    }}
                  >
                    <option value=""></option>
                    <option value="road">ロード</option>
                ...
                <td className="px-2 py-1 w-36">
                  <input
                    ...
                    onChange={e => {
                      const newVal = e.target.value
                      setRecords(prev => prev.map(r => r.id === record.id ? { ...r, date: newVal } : r))
                    }}
                  />
                </td>
                <td className="px-2 py-1">
                  <input
                    ...
                    onChange={e => {
                      const newVal = e.target.value
                      setRecords(prev => prev.map(r => r.id === record.id ? { ...r, comment: newVal } : r))
                    }}
                  />
                </td>
                <td className="px-2 py-1">
